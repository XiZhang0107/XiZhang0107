...
if __name__ == "__main__":
    ...
    parser.add_argument('--lyc2vec', type=str, default=128, help="lyc2vec size")
    ...
    music_vocabulary = np.load(music_vocabulary_file).item()
    ...
    # 打开或创建一个txt文件来记录损失
    loss_log_file = open("addtest_epoch_loss_log.txt", "w")
    ...
    for epoch in range(opt.num_epochs):
        ...
        print(f'Epoch {epoch + 1}/{opt.num_epochs}, Average Train Loss: {average_train_loss}')

        # 记录到txt文件
        loss_log_file.write(f'Epoch {epoch + 1}/{opt.num_epochs}, Average Train Loss: {average_train_loss}\n')
        ...
        with torch.no_grad():
            for iter, testdata in enumerate(test_loader):
                ...
                en_pred, de_pred, _ = model(lyric_input, music_input, en_state_h)
        ...
        print(f'Epoch {epoch + 1}/{opt.num_epochs}, Average Test Loss: {average_test_loss}')

        # 记录测试损失到txt文件
        loss_log_file.write(f'Epoch {epoch + 1}/{opt.num_epochs}, Average Test Loss: {average_test_loss}\n')

        filename = 'GRU_generator_' + 'seqlen_' + str(opt.seqlen) + '_embed_' + str(opt.lyc2vec) + '_epoch_' + str(
            epoch) + '.pkl'
